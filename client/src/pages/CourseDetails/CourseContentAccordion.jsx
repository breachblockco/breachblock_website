...
function CourseContentAccordion({courseContent,key}) {
  ...
  return (
    ...
    >
      <button
        className={`flex items-center justify-between text-neutral-200 w-full text-left my-8 ${
          isOpen ? "bg-neutral-800/40</div>" : ""
        }`}
        onClick={toggleAccordion}
      >
        <span className="text-4xl font-medium leading-50">
          {courseContent.title}
        </span>
        <motion.span
          initial={{ rotate: 0 }}
        ...
        {isOpen && (
          ...
          >
            {courseContent.subtopics.map((subtopic, index) => (
              <div
                key={index}
                className="p-4 text-slate-300 text-3xl hover:text-[#096CD5] transition-all duration-300"
              >
                {subtopic}
              </div>
            ))}
